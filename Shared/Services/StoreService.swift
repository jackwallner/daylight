...
enum DaylightProduct {
    static let monthly = "com.jackwallner.daylight.monthly"
    static let yearly = "com.jackwallner.daylight.yearly"
    static let lifetime = "com.jackwallner.daylight.pro.lifetime"
}
...
extension Package {
    var daylightPackageKind: DaylightPackageKind {
        DaylightPackageKind(package: self)
    }

    var daylightDisplayName: String {
        switch daylightPackageKind {
...
extension Offering {
    var daylightSortedPackages: [Package] {
        availablePackages.sorted {
            let lhsKind = $0.daylightPackageKind
            let rhsKind = $1.daylightPackageKind
...
final class StoreService: NSObject, ObservableObject, PurchasesDelegate {
    ...
    /// gating.
    static let cachedProKey = daylightCachedProKey
    ...
    private let logger = Logger(subsystem: "com.jackwallner.daylight", category: "Store")
    ...
    var yearlyPackage: Package? { packages.first { $0.daylightPackageKind == .yearly } }
    var lifetimePackage: Package? { packages.first { $0.daylightPackageKind == .lifetime } }

    func isEligibleForIntroOffer(_ package: Package) -> Bool {
        guard package.daylightIntroOfferLabel != nil else { return false }
    ...
    func eligibleIntroLabel(for package: Package) -> String? {
        ...
        return package.daylightIntroOfferLabel
    }
    ...
    func restore() async {
        ...
        do {
            ...
            errorMessage = isPro ? nil : "No active Daylight+ purchase was found for this Apple ID."
        } catch {
    ...
    private func loadOffering(forceRefresh: Bool = false) async {
        ...
        do {
            ...
            packages = offering?.daylightSortedPackages ?? []
            errorMessage = nil
            await refreshIntroEligibility()
        } catch {
    ...
    /// local `Daylight.storekit` catalog, and those genuine products are used.
    ...
    private func apply(simulatorProducts products: [StoreProduct]) {
        packages = products
            ...
            .sorted { DaylightPackageKind(package: $0).rawValue < DaylightPackageKind(package: $1).rawValue }
    }

    private static func storeKitTestingProducts() async -> [StoreProduct]? {
        let identifiers: Set<String> = [
            DaylightProduct.monthly, DaylightProduct.yearly, DaylightProduct.lifetime,
        ]
    ...
    /// Same prices and trial as `Daylight.storekit`, for when StoreKit Testing
    ...
    private static func fixtureProducts() -> [StoreProduct] {
        ...
        return [
            TestStoreProduct(
                localizedTitle: "Daylight+ Monthly", price: 5.99, currencyCode: "USD",
                localizedPriceString: "$5.99", productIdentifier: DaylightProduct.monthly,
                productType: .autoRenewableSubscription, localizedDescription: "Daylight+, billed monthly.",
                subscriptionPeriod: .init(value: 1, unit: .month), introductoryDiscount: weekTrial(), locale: locale
            ).toStoreProduct(),
            TestStoreProduct(
                localizedTitle: "Daylight+ Yearly", price: 29.99, currencyCode: "USD",
                localizedPriceString: "$29.99", productIdentifier: DaylightProduct.yearly,
                productType: .autoRenewableSubscription, localizedDescription: "Daylight+, billed yearly.",
                subscriptionPeriod: .init(value: 1, unit: .year), introductoryDiscount: weekTrial(), locale: locale
            ).toStoreProduct(),
            TestStoreProduct(
                localizedTitle: "Daylight+ Lifetime", price: 59.99, currencyCode: "USD",
                localizedPriceString: "$59.99", productIdentifier: DaylightProduct.lifetime,
                productType: .nonConsumable, localizedDescription: "Daylight+, one-time purchase.",
...
enum RevenueCatProbe {
    ...
    static let testStoreKey = "test_ndGYyHqNYymuUPjFPddtaHXqXNn"

    static var appUserID: String {
        ProcessInfo.processInfo.environment["RC_PROBE_USER"] ?? "funnel-probe-daylight"
    }

    static var impressionID: String {
        ProcessInfo.processInfo.environment["RC_PROBE_SURFACE"] ?? "daylight_paywall"
    }
}
#endif
